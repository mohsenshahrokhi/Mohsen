...
function LoginMailForm() {
  ...
  const onSubmit = (values: TLoginMailSchema) => {
    startTransition(async () => {
      // const res = await signIn('EmailCredentials', { ...values, callbackUrl: callbackUrl || '/userDashboard' })
      await signIn("EmailCredentials", { ...values, redirect: false }).then(
        (res) => {
          if (res?.ok) router.push("/userDashboard");
