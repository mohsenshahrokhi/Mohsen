...
function CategoryForm() {
  ...
  const [selectParent, setSelectParent] = useState<SelectValue>({
    id: "",
    name: "",
  });
  const [selectMeasure, setSelectMeasure] = useState<SelectValue>({
    id: "",
    name: "",
  });
  ...
  const items = (
    <div className=" flex flex-col gap-2 w-full border rounded-md p-2">
      <TEInput
        name="eName"
        size="lg"
        label="نام لاتین"
    ...
    <div className=" flex flex-col w-full">
      ...
      >
        ...
        <div className=" flex flex-col max-h-80 overflow-y-auto gap-2 border-t-2 p-2 w-full ">
          ...
          {form?.propertys &&
            form.propertys?.length > 0 &&
            form!.propertys?.map((p, index) => (
              <div
