...
function EditProductForm({ productInfo, searchParams, stringCats, users }: Props) {
    ...
    console.log('form', form.formState.defaultValues, property,)
    ...
    return (
        ...
        >
            ...
            >
                ...
                >
                    {product.propertys.map((field, index) => (

                        <FormControl key={index} component="div" fullWidth sx={{ my: 1 }} variant="outlined">
                            {/* <InputLabel htmlFor={`simple-select-${field.title}`}>{field.title}</InputLabel> */}
                            {/* <Select
                                labelId={`simple-select-lable-${field.title}`}
                                id={`simple-select-${field.title}`}
                                value={field.value}
                                label={field.title}
                                fullWidth
                            // onChange={onChange}
                            // > */}
                            {
                                // field && property![index].values.map((p) => (
