import { getUserByEmail, getUserByPhone, getUserByUsernamePass, updateUser } from "@/lib/controllers/userController";
    ...
    providers: [
        CredentialsProvider({
            ...
            async authorize(credentials, req) {
                ...
                if (!user)
                    throw Error('ایمیل یا گذرواژه اشتباه است!');
                if (!user.verifyMail)
                    throw Error('ابتدا باید حساب کاربری خود را فعال کنید!');
        ...
        CredentialsProvider({
            ...
            async authorize(credentials, req) {
                ...
                connectToMongodb();
                const user = await getUserByUsernamePass(username);
                // if (!user) {
                //     HandleEnqueueSnackbar({ msg: 'email/password mismatch', variant: 'error' })
                //     return null
                // }
                if (!user)
                    throw Error('username/password mismatch');
                const passwordMatch = await bcrypt.compare(password, user.password);
                // if (!passwordMatch) {
                //     HandleEnqueueSnackbar({ msg: 'email/password mismatch', variant: 'error' })
                //     return null
        ...
        CredentialsProvider({
            ...
            credentials: {},
            async authorize(credentials, req) {
                const { phone, verifyPKey } = credentials;
                connectToMongodb();
