...
async function ActivationPage({ params }) {
    const result = await activateUser(params.jwt);
    return (<div className='flex justify-center items-center h-screen w-screen'>{result === 'userNotExist' ? <div><p className=' text-red-500 text-2xl'>کاربر وجود ندارد</p><Link className=' text-blue-500 text-center border border-blue-500 rounded-md p-3' href={'/'}>انتقال به صفحه اصلی</Link></div> :
