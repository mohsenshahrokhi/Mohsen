...
export default function RootLayout({ children, }) {
    ...
    if ((data === null || data === void 0 ? void 0 : data.user) && parseInt(data.user.role) < 1) {
        router.push(`/phone?${stringified}`);
        return null;
    }
    if (moment().isAfter(data === null || data === void 0 ? void 0 : data.expires)) {
        router.push(`/phone?${stringified}`);
        return null;
    }
    ...
    if (status !== "loading") {
        ...
        <div className="flex relative justify-end">
          <div className={` ${sidebarVisible
                ...
                : " translate-x-48 w-0 border-0 p-0 m-0"}  shadow-lg  rounded-md h-[80] flex flex-col transition-all mt-2 ease-in-out duration-300 `}>
            <AdminSidebar />
          </div>
          <div className={` flex ${sidebarVisible
                ? sidebarControl
                    ? " w-[95%] mr-2 "
                    : " w-10/12 "
                : " w-full mr-2 "} transition-all ease-in-out duration-300 my-2 ml-2 p-3 shadow-lg border rounded-md overflow-auto`} onClick={closeSidebar}>
            {children}
          </div>
        </div>
      </div>);
