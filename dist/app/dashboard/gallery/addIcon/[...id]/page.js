...
async function getData({ catId, page, perPage, accessToken }) {
    const params = {
        // parent: 'null'
        limit: perPage,
        skip: perPage * (page - 1)
    ...
    return { gallerys: gallerys, category: category };
}
async function Gallery({ params, searchParams }) {
    ...
    const catProperty = searchParams.catProperty;
    ...
    const imageSvgContainer = (image) => {
        return (<Box component={'div'} className=" flex text-center justify-center">
                <Box component={'div'} className="flex flex-col border">
                    <Box component={'div'} className="p-2 md:flex-shrink-0">

                        <Box component={'div'} className="relative overflow-hidden bg-cover bg-no-repeat">
                            <Image src={`/uploads/${image.type}/${image.url}`} width="148" height="199" priority={true} alt={'galler'} className="rounded-lg"/>
                            <Box component={'div'} className="absolute bottom-0 left-0 right-0 top-0 h-full w-full overflow-hidden bg-[hsla(0,0%,98%,0.15)] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100"></Box>
                            <Box component={'div'} className=' absolute right-1 top-1'>
                                <Checkbox id={image._id} checked={false}/>
                            </Box>
                        </Box>
                    </Box>
                </Box>
    ...
    const videoContainer = (image, key) => {
        return (<Box key={key} component={'div'} className=" flex text-center justify-center">
                <Box component={'div'} className="flex flex-col border">
                    <Box component={'div'} className="p-2 md:flex-shrink-0">
    ...
    const svgContainer = async (image) => {
        ...
        return (<Box component={'div'} className=" flex text-center justify-center">
                ...
                <Box component={'div'} className="flex flex-col border">
                    <Box component={'div'} className="p-2 md:flex-shrink-0">
                        ...
                        <Box component={'div'} className="relative overflow-hidden bg-cover bg-no-repeat">
                            <Lootti animationData={`./public/uploads/${image.type}/${image.url}`} loop={true}/>
                            <Box component={'div'} className="absolute bottom-0 left-0 right-0 top-0 h-full w-full overflow-hidden bg-[hsla(0,0%,98%,0.15)] bg-fixed opacity-0 transition duration-300 ease-in-out hover:opacity-100"></Box>
                            <Box component={'div'} className=' absolute right-1 top-1'>
                                {/* <Checkbox
                                                id={galler._id}
    ...
    return (<Box component={'div'} className="flex flex-col w-full">
            {outOfRange ? (<Box component={'div'}>
                    این صفحه وجود ندارد
                </Box>) : (<Box component={'section'} className="flex relative overflow-y-auto flex-col w-full p-1 my-2">
        ...
        >
            انتخاب عکس
                        ...
                        <Box component={'div'} className="flex items-center">
                            <Button size="medium" variant="contained" color="info" aria-label="add">
                    ...
                    <GalleryBase gallery={gallerys.gallerys} searchParams={stringified} catId={catId} cat={category.category} catProperty={catProperty}/>
