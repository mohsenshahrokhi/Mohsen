...
function GalleryBase({ gallery, searchParams, catId, cat, catProperty }) {
    var _a;
    ...
    function handlePhoto(e) {
        ...
        if (oldImg.includes(e)) {
            ...
            if ((catProperty === 'icon' || catProperty === 'colorIcon') && oldImg.length < 1)
                oldImg.push(e);
                    ...
                    <Box component={'div'} className="p-2 md:flex-shrink-0">
                        ...
                        <Box component={'div'} className="relative overflow-hidden bg-cover bg-no-repeat">
                            ...
                            <Box component={'div'} className=' absolute left-0 top-0'>
                                <Checkbox id={image._id} checked={images.includes(`${image.type}/${image.url}`)} color="info"/>
                            </Box>
                        </Box>
                    </Box>
                </Box>
    ...
    return (<Box component={'div'} className="flex flex-col mt-2">
            <Box component={'div'} className=" flex py-10 h-32">
                {images.length > 0 && <Button color="warning" variant="contained" size="small" onClick={updateCat}>
                    به روز رسانی محصول
                </Button>}
            </Box>
            <Box component={'div'} className="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <Box component={'div'} className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
                    <Box component={'div'} className=" grid grid-cols-4 md:grid-cols-12 p-5 gap-2 overflow-hidden border md:rounded-lg">
                        {gallery.length > 0 && gallery.map((galler, index) => (<Box key={index}>
...
//# sourceMappingURL=GalleryBase.js.map